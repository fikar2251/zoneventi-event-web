...
@section('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Temukan nilai maksimum untuk skala bar
            bars.forEach(bar => {
            ...
            // Atur lebar bar berdasarkan nilai maksimum
            bars.forEach(bar => {
                const value = parseInt(bar.getAttribute('data-value'));
                const widthPercentage = (value / maxValue) * 100;

                // Batasi lebar minimum menjadi 2% agar bar tetap terlihat
                const finalWidth = Math.max(widthPercentage, 2);

                bar.style.width = `${finalWidth}%`;
                bar.style.borderRadius = '5px'; // Tambahkan border radius
            });
        });
    </script>
    <script>
